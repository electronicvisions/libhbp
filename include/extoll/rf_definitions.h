...
struct Reset
{
	...
	bool hicann() const
	{
		return bool(raw >> 1 & 0x1);
	}
	//! Read the `arq` field
	bool arq() const
	{
		return bool(raw >> 2 & 0x1);
	}
	//! Read the `pulse_mem` field
	bool pulse_mem() const
	{
		return bool(raw >> 3 & 0x1);
	}


	//! Set the `core` field
	void core(bool value)
	{
		raw &= ~(0x1ull << 0);
		raw |= (uint64_t(value) & 0x1) << 0;
	}
	//! Set the `hicann` field
	void hicann(bool value)
	{
		raw &= ~(0x1ull << 1);
		raw |= (uint64_t(value) & 0x1) << 1;
	}
	//! Set the `arq` field
	void arq(bool value)
	{
		raw &= ~(0x1ull << 2);
		raw |= (uint64_t(value) & 0x1) << 2;
	}
	//! Set the `pulse_mem` field
	void pulse_mem(bool value)
	{
		raw &= ~(0x1ull << 3);
		raw |= (uint64_t(value) & 0x1) << 3;
	}
...
struct JtagCmd
{
	...
	Type type() const
	{
		return Type(raw >> 0 & 0x7);
	}
	//! Read the `length` field
	uint32_t length() const
	{
		return uint32_t(raw >> 3 & 0x3ff);
	}
	...
	{
		raw &= ~(0x7ull << 0);
		raw |= (uint64_t(value) & 0x7) << 0;
	}
	//! Set the `length` field
	void length(uint32_t value)
	{
		raw &= ~(0x3ffull << 3);
		raw |= (uint64_t(value) & 0x3ff) << 3;
	}
	//! Set the `pause` field
	void pause(bool value)
	{
		raw &= ~(0x1ull << 13);
		raw |= (uint64_t(value) & 0x1) << 13;
	}
	//! Set the `execute` field
	void execute(bool value)
	{
		raw &= ~(0x1ull << 14);
		raw |= (uint64_t(value) & 0x1) << 14;
	}
...
struct JtagSend
{
	...
	JtagSend(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct JtagReceive
{
	...
	JtagReceive() = default;
	//! Initialize the single field with a specific value
	JtagReceive(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannIfState
{
	...
	uint32_t channel_status() const
	{
		return uint32_t(raw >> 0 & 0xff);
	}
	//! Read the `crc_count` field
	uint32_t crc_count() const
	{
		return uint32_t(raw >> 8 & 0xff);
	}
	//! Read the `systime` field
	uint32_t systime() const
	{
		return uint32_t(raw >> 16 & 0x3fff);
	}
	//! Read the `init_state` field
	uint32_t init_state() const
	{
		return uint32_t(raw >> 30 & 0x3f);
	}
	//! Read the `rx_data` field
	uint32_t rx_data() const
	{
		return uint32_t(raw >> 36 & 0xff);
	}
	//! Read the `tx_data` field
	uint32_t tx_data() const
	{
		return uint32_t(raw >> 44 & 0xff);
	}


	//! Set the `channel_status` field
	void channel_status(uint32_t value)
	{
		raw &= ~(0xffull << 0);
		raw |= (uint64_t(value) & 0xff) << 0;
	}
	//! Set the `crc_count` field
	void crc_count(uint32_t value)
	{
		raw &= ~(0xffull << 8);
		raw |= (uint64_t(value) & 0xff) << 8;
	}
	//! Set the `systime` field
	void systime(uint32_t value)
	{
		raw &= ~(0x3fffull << 16);
		raw |= (uint64_t(value) & 0x3fff) << 16;
	}
	//! Set the `init_state` field
	void init_state(uint32_t value)
	{
		raw &= ~(0x3full << 30);
		raw |= (uint64_t(value) & 0x3f) << 30;
	}
	//! Set the `rx_data` field
	void rx_data(uint32_t value)
	{
		raw &= ~(0xffull << 36);
		raw |= (uint64_t(value) & 0xff) << 36;
	}
	//! Set the `tx_data` field
	void tx_data(uint32_t value)
	{
		raw &= ~(0xffull << 44);
		raw |= (uint64_t(value) & 0xff) << 44;
	}
...
struct SetHicannIfState
{
	...
	uint32_t data_delay() const
	{
		return uint32_t(raw >> 0 & 0x1f);
	}
	//! Read the `data_delay_dout` field
	uint32_t data_delay_dout() const
	{
		return uint32_t(raw >> 5 & 0x1f);
	}


	//! Set the `data_delay` field
	void data_delay(uint32_t value)
	{
		raw &= ~(0x1full << 0);
		raw |= (uint64_t(value) & 0x1f) << 0;
	}
	//! Set the `data_delay_dout` field
	void data_delay_dout(uint32_t value)
	{
		raw &= ~(0x1full << 5);
		raw |= (uint64_t(value) & 0x1f) << 5;
	}
...
struct HicannIfRxConfig
{
	...
	//! Initialize the single field with a specific value
	HicannIfRxConfig(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannIfRxPulse
{
	...
	uint32_t data() const
	{
		return uint32_t(raw >> 0 & 0xffffff);
	}
...
struct HicannIfConfig
{
	...
	HicannIfConfig(
	    bool start_link_,
	    bool loopback_enable_,
	    bool auto_init_,
	    bool init_master_,
	    bool use_timestamp_,
	    bool auto_limit_,
	    bool pulse_protocol_,
	    bool config_protocol_,
	    uint32_t heap_mode_,
	    uint32_t limit_,
	    bool dc_coding_,
	    bool invert_deserializer_data_)
	{
	...
	bool start_link() const
	{
		return bool(raw >> 0 & 0x1);
	}
	//! Read the `loopback_enable` field
	bool loopback_enable() const
	{
		return bool(raw >> 1 & 0x1);
	}
	...
	{
		return bool(raw >> 4 & 0x1);
	}
	//! Read the `auto_limit` field
	bool auto_limit() const
	{
		return bool(raw >> 5 & 0x1);
	}
	//! Read the `pulse_protocol` field
	bool pulse_protocol() const
	{
		return bool(raw >> 6 & 0x1);
	}
	//! Read the `config_protocol` field
	bool config_protocol() const
	{
		return bool(raw >> 7 & 0x1);
	}
	//! Read the `heap_mode` field
	uint32_t heap_mode() const
	{
		return uint32_t(raw >> 8 & 0xff);
	}
	...
	{
		return uint32_t(raw >> 16 & 0x7ff);
	}
	//! Read the `dc_coding` field
	bool dc_coding() const
	{
		return bool(raw >> 27 & 0x1);
	}
	//! Read the `invert_deserializer_data` field
	bool invert_deserializer_data() const
	{
		return bool(raw >> 28 & 0x1);
	}


	//! Set the `start_link` field
	void start_link(bool value)
	{
		raw &= ~(0x1ull << 0);
		raw |= (uint64_t(value) & 0x1) << 0;
	}
	...
	void auto_init(bool value)
	{
		raw &= ~(0x1ull << 2);
		raw |= (uint64_t(value) & 0x1) << 2;
	}
	//! Set the `init_master` field
	void init_master(bool value)
	{
		raw &= ~(0x1ull << 3);
		raw |= (uint64_t(value) & 0x1) << 3;
	}
	//! Set the `use_timestamp` field
	void use_timestamp(bool value)
	{
		raw &= ~(0x1ull << 4);
		raw |= (uint64_t(value) & 0x1) << 4;
	}
	//! Set the `auto_limit` field
	void auto_limit(bool value)
	{
		raw &= ~(0x1ull << 5);
		raw |= (uint64_t(value) & 0x1) << 5;
	}
	//! Set the `pulse_protocol` field
	void pulse_protocol(bool value)
	{
		raw &= ~(0x1ull << 6);
		raw |= (uint64_t(value) & 0x1) << 6;
	}
	//! Set the `config_protocol` field
	void config_protocol(bool value)
	{
		raw &= ~(0x1ull << 7);
		raw |= (uint64_t(value) & 0x1) << 7;
	}
	//! Set the `heap_mode` field
	void heap_mode(uint32_t value)
	{
		raw &= ~(0xffull << 8);
		raw |= (uint64_t(value) & 0xff) << 8;
	}
	...
	{
		raw &= ~(0x7ffull << 16);
		raw |= (uint64_t(value) & 0x7ff) << 16;
	}
	//! Set the `dc_coding` field
	void dc_coding(bool value)
	{
		raw &= ~(0x1ull << 27);
		raw |= (uint64_t(value) & 0x1) << 27;
	}
	//! Set the `invert_deserializer_data` field
	void invert_deserializer_data(bool value)
	{
		raw &= ~(0x1ull << 28);
		raw |= (uint64_t(value) & 0x1) << 28;
	}
...
struct HicannIfControls
{
	...
	//! Initialize all fields with a specific value
	HicannIfControls(
	    bool tx_hicann_pulse_enable_,
	    bool tx_hicann_config_enable_,
	    bool routing_data_enable_,
	    bool channel_reset_,
	    bool crc_count_reset_)
	{
	...
	bool tx_hicann_pulse_enable() const
	{
		return bool(raw >> 0 & 0x1);
	}
	//! Read the `tx_hicann_config_enable` field
	bool tx_hicann_config_enable() const
	{
		return bool(raw >> 1 & 0x1);
	}
	//! Read the `routing_data_enable` field
	bool routing_data_enable() const
	{
		return bool(raw >> 2 & 0x1);
	}
	//! Read the `channel_reset` field
	bool channel_reset() const
	{
		return bool(raw >> 3 & 0x1);
	}
	//! Read the `crc_count_reset` field
	bool crc_count_reset() const
	{
		return bool(raw >> 4 & 0x1);
	}
	...
	void tx_hicann_pulse_enable(bool value)
	{
		raw &= ~(0x1ull << 0);
		raw |= (uint64_t(value) & 0x1) << 0;
	}
	//! Set the `tx_hicann_config_enable` field
	void tx_hicann_config_enable(bool value)
	{
		raw &= ~(0x1ull << 1);
		raw |= (uint64_t(value) & 0x1) << 1;
	}
	//! Set the `routing_data_enable` field
	void routing_data_enable(bool value)
	{
		raw &= ~(0x1ull << 2);
		raw |= (uint64_t(value) & 0x1) << 2;
	}
	//! Set the `channel_reset` field
	void channel_reset(bool value)
	{
		raw &= ~(0x1ull << 3);
		raw |= (uint64_t(value) & 0x1) << 3;
	}
	//! Set the `crc_count_reset` field
	void crc_count_reset(bool value)
	{
		raw &= ~(0x1ull << 4);
		raw |= (uint64_t(value) & 0x1) << 4;
	}
...
struct HicannIfTxData
{
	...
	//! Initialize the single field with a specific value
	HicannIfTxData(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannIfNeuronAddressFilter
{
	...
	uint32_t negative_filter_mask() const
	{
		return uint32_t(raw >> 0 & 0x1ff);
	}
	//! Read the `positive_filter_mask` field
	uint32_t positive_filter_mask() const
	{
		return uint32_t(raw >> 9 & 0x1ff);
	}


	//! Set the `negative_filter_mask` field
	void negative_filter_mask(uint32_t value)
	{
		raw &= ~(0x1ffull << 0);
		raw |= (uint64_t(value) & 0x1ff) << 0;
	}
	//! Set the `positive_filter_mask` field
	void positive_filter_mask(uint32_t value)
	{
		raw &= ~(0x1ffull << 9);
		raw |= (uint64_t(value) & 0x1ff) << 9;
	}
...
struct HicannPacketGen
{
	...
	{
		return uint32_t(raw >> 0 & 0x1ffffff);
	}
	//! Read the `error` field
	bool error() const
	{
		return bool(raw >> 25 & 0x1);
	}
	//! Read the `enable` field
	bool enable() const
	{
		return bool(raw >> 26 & 0x1);
	}
	...
	{
		raw &= ~(0x1ffffffull << 0);
		raw |= (uint64_t(value) & 0x1ffffff) << 0;
	}
	//! Set the `error` field
	void error(bool value)
	{
		raw &= ~(0x1ull << 25);
		raw |= (uint64_t(value) & 0x1) << 25;
	}
	//! Set the `enable` field
	void enable(bool value)
	{
		raw &= ~(0x1ull << 26);
		raw |= (uint64_t(value) & 0x1) << 26;
	}
...
struct HicannTxIfPower
{
	...
	uint32_t channel0() const
	{
		return uint32_t(raw >> 0 & 0x3);
	}
	//! Read the `channel1` field
	uint32_t channel1() const
	{
		return uint32_t(raw >> 2 & 0x3);
	}
	//! Read the `channel2` field
	uint32_t channel2() const
	{
		return uint32_t(raw >> 4 & 0x3);
	}
	//! Read the `channel3` field
	uint32_t channel3() const
	{
		return uint32_t(raw >> 6 & 0x3);
	}
	//! Read the `channel4` field
	uint32_t channel4() const
	{
		return uint32_t(raw >> 8 & 0x3);
	}
	//! Read the `channel5` field
	uint32_t channel5() const
	{
		return uint32_t(raw >> 10 & 0x3);
	}
	//! Read the `channel6` field
	uint32_t channel6() const
	{
		return uint32_t(raw >> 12 & 0x3);
	}
	//! Read the `channel7` field
	uint32_t channel7() const
	{
		return uint32_t(raw >> 14 & 0x3);
	}


	//! Set the `channel0` field
	void channel0(uint32_t value)
	{
		raw &= ~(0x3ull << 0);
		raw |= (uint64_t(value) & 0x3) << 0;
	}
	//! Set the `channel1` field
	void channel1(uint32_t value)
	{
		raw &= ~(0x3ull << 2);
		raw |= (uint64_t(value) & 0x3) << 2;
	}
	//! Set the `channel2` field
	void channel2(uint32_t value)
	{
		raw &= ~(0x3ull << 4);
		raw |= (uint64_t(value) & 0x3) << 4;
	}
	//! Set the `channel3` field
	void channel3(uint32_t value)
	{
		raw &= ~(0x3ull << 6);
		raw |= (uint64_t(value) & 0x3) << 6;
	}
	//! Set the `channel4` field
	void channel4(uint32_t value)
	{
		raw &= ~(0x3ull << 8);
		raw |= (uint64_t(value) & 0x3) << 8;
	}
	//! Set the `channel5` field
	void channel5(uint32_t value)
	{
		raw &= ~(0x3ull << 10);
		raw |= (uint64_t(value) & 0x3) << 10;
	}
	//! Set the `channel6` field
	void channel6(uint32_t value)
	{
		raw &= ~(0x3ull << 12);
		raw |= (uint64_t(value) & 0x3) << 12;
	}
	//! Set the `channel7` field
	void channel7(uint32_t value)
	{
		raw &= ~(0x3ull << 14);
		raw |= (uint64_t(value) & 0x3) << 14;
	}
...
struct TestControlConfig
{
	...
	uint32_t quad_words() const
	{
		return uint32_t(raw >> 0 & 0xff);
	}
	//! Read the `pause_time` field
	uint32_t pause_time() const
	{
		return uint32_t(raw >> 8 & 0xff);
	}
	...
	bool count_up() const
	{
		return bool(raw >> 16 & 0x1);
	}


	//! Set the `quad_words` field
	void quad_words(uint32_t value)
	{
		raw &= ~(0xffull << 0);
		raw |= (uint64_t(value) & 0xff) << 0;
	}
	//! Set the `pause_time` field
	void pause_time(uint32_t value)
	{
		raw &= ~(0xffull << 8);
		raw |= (uint64_t(value) & 0xff) << 8;
	}
	//! Set the `count_up` field
	void count_up(bool value)
	{
		raw &= ~(0x1ull << 16);
		raw |= (uint64_t(value) & 0x1) << 16;
	}
...
struct TestControlData
{
	...
	//! Initialize the single field with a specific value
	TestControlData(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct AlHicannConfigReceived
{
	...
	//! Initialize the single field with a specific value
	AlHicannConfigReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlFpgaConfigReceived
{
	...
	//! Initialize the single field with a specific value
	AlFpgaConfigReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlPlaybackDataReceived
{
	...
	//! Initialize the single field with a specific value
	AlPlaybackDataReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlJtagReceived
{
	...
	//! Initialize the single field with a specific value
	AlJtagReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlErrorUnknownTypeReceived
{
	...
	//! Initialize the single field with a specific value
	AlErrorUnknownTypeReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlHicannConfigSent
{
	...
	//! Initialize the single field with a specific value
	AlHicannConfigSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlFpgaConfigSent
{
	...
	//! Initialize the single field with a specific value
	AlFpgaConfigSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlTracePulseSent
{
	...
	//! Initialize the single field with a specific value
	AlTracePulseSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlJtagSent
{
	...
	AlJtagSent() = default;
	//! Initialize the single field with a specific value
	AlJtagSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct AlErrorUnknownTypeSent
{
	...
	//! Initialize the single field with a specific value
	AlErrorUnknownTypeSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct TraceBufferSize
{
	...
	{
		raw &= ~(0xffffffffull << 0);
		raw |= (uint64_t(value) & 0xffffffff) << 0;
	}
...
struct TraceBufferCounter
{
	...
	{
		return uint32_t(raw >> 32 & 0xffff);
	}
	//! Read the `wraps` field
	uint32_t wraps() const
	{
		return uint32_t(raw >> 48 & 0xffff);
	}
	...
	{
		raw &= ~(0xffffull << 32);
		raw |= (uint64_t(value) & 0xffff) << 32;
	}
	//! Set the `wraps` field
	void wraps(uint32_t value)
	{
		raw &= ~(0xffffull << 48);
		raw |= (uint64_t(value) & 0xffff) << 48;
	}
...
struct TraceBufferCurrentAddress
{
	...
	//! Initialize the single field with a specific value
	TraceBufferCurrentAddress(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct TraceBufferEndAddress
{
	...
	//! Initialize the single field with a specific value
	TraceBufferEndAddress(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannBufferStart
{
	...
	//! Initialize the single field with a specific value
	HicannBufferStart(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannBufferCurrentAddress
{
	...
	//! Initialize the single field with a specific value
	HicannBufferCurrentAddress(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct HicannBufferEndAddress
{
	...
	//! Initialize the single field with a specific value
	HicannBufferEndAddress(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct RraPutReceived
{
	...
	//! Initialize the single field with a specific value
	RraPutReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct RraGetReceived
{
	...
	//! Initialize the single field with a specific value
	RraGetReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct RmaPutReceived
{
	...
	//! Initialize the single field with a specific value
	RmaPutReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct RmaNotificationReceived
{
	...
	//! Initialize the single field with a specific value
	RmaNotificationReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct PlaybackReceived
{
	...
	//! Initialize the single field with a specific value
	PlaybackReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct FpgaConfigReceived
{
	...
	//! Initialize the single field with a specific value
	FpgaConfigReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct HicannConfigReceived
{
	...
	//! Initialize the single field with a specific value
	HicannConfigReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct JtagDataReceived
{
	...
	//! Initialize the single field with a specific value
	JtagDataReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct NeighbourDataReceived
{
	...
	//! Initialize the single field with a specific value
	NeighbourDataReceived(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidCommand
{
	...
	//! Initialize the single field with a specific value
	InvalidCommand(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidType
{
	...
	InvalidType() = default;
	//! Initialize the single field with a specific value
	InvalidType(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidPayloadSize
{
	...
	//! Initialize the single field with a specific value
	InvalidPayloadSize(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidFields
{
	...
	//! Initialize the single field with a specific value
	InvalidFields(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidMode
{
	...
	InvalidMode() = default;
	//! Initialize the single field with a specific value
	InvalidMode(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct InvalidRraAddress
{
	...
	//! Initialize the single field with a specific value
	InvalidRraAddress(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct ConfigAddressReinit
{
	...
	//! Initialize the single field with a specific value
	ConfigAddressReinit(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct UndefinedHost
{
	...
	//! Initialize the single field with a specific value
	UndefinedHost(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct RraGetResponsesSent
{
	...
	//! Initialize the single field with a specific value
	RraGetResponsesSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct RmaPutRequestsSent
{
	...
	//! Initialize the single field with a specific value
	RmaPutRequestsSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct ReceiptNotificationPutRequestsSent
{
	...
	//! Initialize the single field with a specific value
	ReceiptNotificationPutRequestsSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct PayloadNotificationPutRequestsSent
{
	...
	//! Initialize the single field with a specific value
	PayloadNotificationPutRequestsSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct TraceDataSent
{
	...
	//! Initialize the single field with a specific value
	TraceDataSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct FpagConfigSent
{
	...
	//! Initialize the single field with a specific value
	FpagConfigSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct HicannConfigSent
{
	...
	//! Initialize the single field with a specific value
	HicannConfigSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct JtagDataSent
{
	...
	//! Initialize the single field with a specific value
	JtagDataSent(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct NeighbourPulseSend
{
	...
	//! Initialize the single field with a specific value
	NeighbourPulseSend(uint64_t value) : raw(value & 0xffffffffffff) {}
...
struct ResponderCounterReset
{
	...
	uint64_t count() const
	{
		return uint64_t(raw >> 1 & 0xffffffffffff);
	}
	...
	void count(uint64_t value)
	{
		raw &= ~(0xffffffffffffull << 1);
		raw |= (uint64_t(value) & 0xffffffffffff) << 1;
	}
...
struct ConfigResponse
{
	...
	//! Initialize the single field with a specific value
	ConfigResponse(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct TraceNotificationBehaviour
{
	...
	{
		raw &= ~(0xffffffffull << 32);
		raw |= (uint64_t(value) & 0xffffffff) << 32;
	}
...
struct Version
{
	...
	Version() = default;
	//! Initialize the single field with a specific value
	Version(uint64_t value) : raw(value & 0xffffffffffffffff) {}
...
struct Driver
{
	...
	Driver() = default;
	//! Initialize the single field with a specific value
	Driver(uint64_t value) : raw(value & 0xffffffffffffffff) {}
