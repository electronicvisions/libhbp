...
public:
	...
	typename std::enable_if<JR::READABLE && !JR::WRITABLE, uint64_t>::type read(uint8_t hicann);
	...
	//! \endcode
	template <typename JR>
	typename std::enable_if<JR::WRITABLE && !JR::READABLE, void>::type write(
	    uint64_t, uint8_t hicann);
	...
	//! \endcode
	template <typename JR>
	typename std::enable_if<JR::WRITABLE && JR::READABLE, uint64_t>::type write(
	    uint64_t, uint8_t hicann);
...
template <typename JR>
typename std::enable_if<JR::READABLE && !JR::WRITABLE, uint64_t>::type JTag::read(uint8_t hicann)
{
...
template <typename JR>
typename std::enable_if<JR::WRITABLE && !JR::READABLE, void>::type JTag::write(
    uint64_t value, uint8_t hicann)
{
	set_command(JR::ADDRESS, hicann);
...
template <typename JR>
typename std::enable_if<JR::WRITABLE && JR::READABLE, uint64_t>::type JTag::write(
    uint64_t value, uint8_t hicann)
{
	set_command(JR::ADDRESS, hicann);
