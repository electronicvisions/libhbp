...
RegisterFile::RegisterFile(Endpoint& connection) : _connection(connection), _status(RMA2_SUCCESS) {}

uint64_t RegisterFile::read_noblock(RMA2_NLA address) const
{
	_status = rma2_post_get_qw(
	    _connection.rra.port, _connection.rra.handle, _connection.rra_buffer.region(), 0, 8,
	    address, RMA2_COMPLETER_NOTIFICATION, RMA2_CMD_DEFAULT);
	throw_on_error<FailedToRead>(_status, _connection.node, address);

	usleep(1000);

	return _connection.rra_buffer[0];
}
...
void RegisterFile::write_noblock(RMA2_NLA address, uint64_t value)
{
	_status = rma2_post_immediate_put(
	    _connection.rra.port, _connection.rra.handle, 8, value, address,
