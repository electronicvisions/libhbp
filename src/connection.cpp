...
static void wait_with_timeout(
    RMA2_Port port, std::chrono::duration<double> timeout = std::chrono::milliseconds(1))
{
...
Endpoint::Endpoint(RMA2_Nodeid n) :
    ...
    rra(n, true),
    rma(n, false),
    rra_buffer(rra.port, 1),
    rma_buffer(rma.port, 1),
    poller(rma.port),
    ...
    hicann_config(rma.port, rma.handle, 100, Extoll::HICANN_CONFIG, poller)
{
	RMA2_ERROR status = rma2_post_get_qw(
	    rra.port, rra.handle, rra_buffer.region(), 0, 8, 0x8000, RMA2_COMPLETER_NOTIFICATION,
	    RMA2_CMD_DEFAULT);
...
void Endpoint::perform_write_test(RMA2_NLA address, uint64_t value)
{
	RMA2_ERROR status = rma2_post_immediate_put(
	    rra.port, rra.handle, 8, value, address, RMA2_COMPLETER_NOTIFICATION, RMA2_CMD_DEFAULT);
...
{
	RMA2_ERROR status = rma2_post_get_qw(
	    rra.port, rra.handle, rra_buffer.region(), 0, 8, address, RMA2_COMPLETER_NOTIFICATION,
	    RMA2_CMD_DEFAULT);
