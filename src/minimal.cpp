...
void virtual_memory(Connection c)
{
	...
	CHECK(rma2_post_get_qw(
	    c.port, c.handle, region, 0, 8, 0x8000, RMA2_ALL_NOTIFICATIONS, RMA2_CMD_DEFAULT));
...
void physical_memory(Connection c)
{
	...
	auto buffer =
	    static_cast<uint64_t*>(mmap(nullptr, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
	...
	CHECK(rma2_post_get_qw_direct(
	    c.port, c.handle, nla, 8, 0x8000, RMA2_ALL_NOTIFICATIONS, RMA2_CMD_DEFAULT));
