...
Hicann::Hicann(Endpoint& connection, uint8_t number) : RegisterFile(connection), _number(number) {}
...
void Hicann::write(uint16_t address, uint32_t value)
{
	...
	rma2_post_immediate_put(
	    rma.port, rma.handle, 8, payload.raw, CONFIG_ADDRESS, RMA2_COMPLETER_NOTIFICATION,
	    RMA2_CMD_DEFAULT);
...
void Hicann::send(uint64_t data)
{
	...
	rma2_post_immediate_put(
	    rma.port, rma.handle, 8, data, CONFIG_ADDRESS, RMA2_COMPLETER_NOTIFICATION,
	    RMA2_CMD_DEFAULT);
